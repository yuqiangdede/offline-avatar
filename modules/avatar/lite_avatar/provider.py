# File: offline-avatar/modules/avatar/lite_avatar/provider.py
...
from modules.core.interfaces import AvatarProvider
...
class LiteAvatarProvider(AvatarProvider):
    def __init__(
        ...
        self.delete_generated_mp4 = bool(delete_generated_mp4)
        self.delete_generated_audio = bool(delete_generated_audio)
        ...
        self.generated_mp4_dir = base / "runtime" / "logs" / "generated-mp4"
        self.generated_audio_dir = base / "runtime" / "logs" / "generated-audio"
        ...
        if not self.delete_generated_mp4:
            self.generated_mp4_dir.mkdir(parents=True, exist_ok=True)
        if not self.delete_generated_audio:
            self.generated_audio_dir.mkdir(parents=True, exist_ok=True)

    def _preserve_mp4_for_debug(self, mp4_path: Path) -> None:
        if self.delete_generated_mp4:
            return
        if not mp4_path.exists():
            return

        ts = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
        dst = self.generated_mp4_dir / f"{ts}_{mp4_path.name}"
        try:
            shutil.copy2(mp4_path, dst)
            logger.info(
                "Lite-Avatar preserved mp4 for debug: src=%s dst=%s delete_generated_mp4=%s",
                mp4_path,
                dst,
                self.delete_generated_mp4,
            )
        except Exception:
            logger.exception("Lite-Avatar preserve mp4 failed: src=%s", mp4_path)

    def _preserve_audio_for_debug(self, pcm_s16le: bytes, sample_rate: int) -> None:
        if self.delete_generated_audio:
            return
        if not pcm_s16le:
            return
        ts = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
        dst = self.generated_audio_dir / f"{ts}_input.wav"
        try:
            self._write_wav(dst, pcm_s16le, sample_rate)
            logger.info(
                "Lite-Avatar preserved audio for debug: dst=%s delete_generated_audio=%s",
                dst,
                self.delete_generated_audio,
            )
        except Exception:
            logger.exception("Lite-Avatar preserve audio failed: dst=%s", dst)
    ...
    def _normalize_python_script_arg(self, cmd_parts: list[str]) -> list[str]:
        ...
        Example:
          cwd = .../modules/avatar/lite_avatar
          cmd = python modules/avatar/lite_avatar/lite_avatar.py
        """
        if len(cmd_parts) < 2:
            return cmd_parts
    ...
    def _resolve_ffmpeg_bin(self) -> str | None:
        ...
        candidates = []
        candidates.extend(env_path.glob("third_party/ffmpeg/bin/ffmpeg.exe"))
        ...
        with tempfile.TemporaryDirectory(dir=self.temp_dir) as td:
            ...
            if videos:
                ...
                try:
                    ...
                    container.close()
                self._preserve_mp4_for_debug(out_path)
                return
            ...
            if frame_dir.exists():
                ...
                if muxed:
                    ...
                    try:
                        ...
                        container.close()
                    self._preserve_mp4_for_debug(muxed)
                    return
    ...
    def render(self, pcm_s16le: bytes, sample_rate: int, avatar_asset: str) -> Iterator[VideoFrame]:
        self._preserve_audio_for_debug(pcm_s16le=pcm_s16le, sample_rate=sample_rate)
