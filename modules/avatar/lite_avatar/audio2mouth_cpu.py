...
class Audio2Mouth(object):
    def __init__(self, use_gpu, gpu_index=0):

        self.p_list = [str(ii) for ii in range(32)]

        model_path = './weights/model_1.onnx'
        available = onnxruntime.get_available_providers()
        use_cuda = bool(use_gpu and "CUDAExecutionProvider" in available and torch.cuda.is_available())
        providers = (
            [("CUDAExecutionProvider", {"device_id": int(gpu_index)}), "CPUExecutionProvider"]
            if use_cuda
            else ["CPUExecutionProvider"]
        )
        self.audio2mouth_model = onnxruntime.InferenceSession(model_path, providers=providers)
        print(
            "Audio2Mouth provider:",
            self.audio2mouth_model.get_providers(),
            "requested_gpu:",
            use_gpu,
            "gpu_index:",
            int(gpu_index),
