...
class liteAvatar(object):
    def __init__(self,
        ...
        if self.use_gpu and self.gpu_index >= torch.cuda.device_count():
            logger.warning(
                "gpu_index={} out of range(device_count={}), fallback to 0",
                self.gpu_index,
                torch.cuda.device_count(),
            )
            self.gpu_index = 0
        self.device = f"cuda:{self.gpu_index}" if self.use_gpu else "cpu"
        os.environ["LITE_AVATAR_ASR_DEVICE"] = "cuda" if self.use_gpu else "cpu"
        logger.info("liteAvatar device: {} (use_gpu={}, gpu_index={})", self.device, self.use_gpu, self.gpu_index)
        
        s = time.time()
        from audio2mouth_cpu import Audio2Mouth
        
        self.audio2mouth = Audio2Mouth(self.use_gpu, gpu_index=self.gpu_index)
    ...
    def handle(self, audio_file_path, result_dir, param_res=None):
        ...
        if ffmpeg_bin is None:
            logger.warning("ffmpeg not found in PATH, skip mp4 mux and keep tmp_frames")
            return

        output_mp4 = os.path.join(result_dir, "test_demo.mp4")
        cmd_nvenc = [
            ffmpeg_bin,
            "-r",
            str(max(1, int(self.fps))),
            "-i",
            os.path.join(tmp_frame_dir, "%05d.jpg"),
            "-i",
            audio_file_path,
            "-framerate",
            str(max(1, int(self.fps))),
        ...
        if proc.returncode != 0:
            ...
            if proc.returncode == 0:
                logger.info("ffmpeg mux fallback to cpu encoder: libx264")
        if proc.returncode != 0:
            logger.error(
                "ffmpeg mux failed(returncode={}): stderr={}",
                proc.returncode,
                (proc.stderr or "").strip()[:1000],
            )
            return

        logger.info("ffmpeg mux success: {}", output_mp4)
    ...
    def _resolve_ffmpeg_path(self):
        ...
        ffmpeg_env = (os.environ.get("FFMPEG_PATH") or "").strip().strip('"').strip("'")
        if ffmpeg_env and os.path.exists(ffmpeg_env):
            return ffmpeg_env

        this_dir = os.path.dirname(os.path.abspath(__file__))
        models_dir = os.path.dirname(this_dir)
        for root in [models_dir, os.getcwd()]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_gpu', action='store_true', help='Enable CUDA for torch/onnx (fallback to CPU if unavailable).')
    ...
    lite_avatar = liteAvatar(
        ...
        ffmpeg_path=str(args.ffmpeg_path or "").strip(),
    )
